...
# Function to visualize DNA sequence with primers and product regions
...
# Add sequence as dots with transparency
    trace = go.Scatter(
        x=x_vals,
        y=y_vals,
        mode="markers",
        marker=dict(
            ...
            color=[f"rgba({int(mcolors.to_rgb(base_colors[base])[0]*255)},"
                ...
                f"{int(mcolors.to_rgb(base_colors[base])[2]*255)},0.2)" for base in sequence],  # 20% opacity
        ),
        hoverinfo="text",
    ...
    # Add primers and product regions
    shapes = []
    primer_hover_text = []
    for index, primer in enumerate(primer_pairs):
        ...
        # Product region: starts after the upstream primer and ends before the downstream primer
        product_start = left_end + 1
        product_end = right_start - 1

        # Add shapes for the product region, handling multiple rows
        ...
        # Primer hover text
        primer_hover_text.append(f"Primer Pair {primer['Index']}<br>"
